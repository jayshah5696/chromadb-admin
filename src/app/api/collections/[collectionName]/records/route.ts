...
export async function GET(request: Request, { params }: { params: { collectionName: string } }) {
  ...
  try {
    ...
    const [data, totalCount] = await Promise.all([
      fetchRecords(connectionString, auth, params.collectionName, page, tenant, database, apiVersion, where),
      countRecord(connectionString, auth, params.collectionName, tenant, database, apiVersion, where),
    ])
